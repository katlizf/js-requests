...
const sayHelloChanges = () => {
    document.querySelector('#say-hello-button').style.color = 'white';
    document.querySelector('#say-hello-button').style.backgroundColor = 'black'
}

sayHelloButton.addEventListener('mouseover', sayHelloChanges)
...
const changeBack = () => {
    document.querySelector('#say-hello-button').style.color = 'black'
    document.querySelector('#say-hello-button').style.backgroundColor = '#EFEFEF'
}
...
const ohMy = () => {
    ...
    .then((res) => {
        for(let i = 0; i <res.data.length; i++) {
            ...
            document.querySelector('p').textContent = res.data[i]
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/katie')
    .then((res) => {
...
const request = () => {
    axios.get(`http://localhost:3000/query-test?animal=lion`)
    .then((res) => {
        console.log(res.data)
    })
    .catch(err => {
...
document.querySelector('#query-button').addEventListener(`click`, request)
